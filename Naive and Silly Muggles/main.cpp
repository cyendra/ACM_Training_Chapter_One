...
bool solve(point a,point b,point c,point d){
    ...
    circle cc(p,r);
    if ( cc.relation(c)!=0&&cc.relation(d)==0 )return true;
    else return false;
}
int main()
{
    ...
    for (int cas=1;cas<=T;cas++){
        ...
        c.input();
        d.input();
        bool ans= solve(a,b,c,d)||solve(a,c,b,d)||solve(b,c,a,d);
        bool tmp;
        circle cc(a,b,c);
        if (cc.relation(d)==0) tmp=true;
        else tmp=false;
        ans=ans||tmp;
        //cerr<<cc.p.x<<" "<<cc.p.y<<" "<<cc.r<<endl;
        cout<<"Case #"<<cas<<": ";
        //int res=cc.relation(d);
